...
class ExtractRequest(BaseModel):
    ...
    url: HttpUrl = Field(
        ..., description="Company website URL", examples=["https://www.anthropic.com"]
    )
    use_simple_extraction: bool = Field(
        default=True, description="Use simple HTML extraction (vs full browser)"
    )
...
async def extract_company(req: ExtractRequest) -> CompanyInfo:
    ...
    try:
        ...
        csv_content += (
            f'"{company_info["url"]}","{company_info["company"]}",'
            f'"{company_info["industry"]}","{company_info["description"]}",'
            f'{company_info["employee_count"] or ""}\n'
        )
...
async def root():
    ...
    return {"service": "Extract Company", "status": "ready", "version": "1.0.0"}
